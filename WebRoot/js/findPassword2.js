...
function checkAnswer(){
	var answer1 = encodeURI(encodeURI(get("answer1").value));
	var answer2 = encodeURI(encodeURI(get("answer2").value));
	var answer3 = encodeURI(encodeURI(get("answer3").value));
	if(answer1==""||answer2==""||answer3==""){
		get("status2").innerHTML = "您还有问题没有回答！";
		get("status2").style.display = "block";
	}else{
		createXmlHttp();
		console.log("password_check?username="+username+"&answer1="+answer1+"&answer2="+answer2+"&answer3="+answer3);
...
function setPassword(){
	...
	if(password==""){
		...
		createXmlHttp();
		xmlHttp.open("POST","password_set?username="+username+"&type="+type+"&password="+password,true);			
