...
function friendphotos(){
	createXmlHttp();
	xmlHttp.open("GET", "friendbloglist.action",true);			
...
function responseTo1(){
	if(xmlHttp.readyState==4){
		...
		var html = "";
		var bloglist = get("bloglist");	//分页所在的div，id为photolist
		console.log(obj);
		var blogs = obj.fbloglist.fbloglist;
		
		bloglist.innerHTML = "";	
		total = Math.ceil(blogs.length/2);		//计算分页的总页数
		...
		for(var tempindex=0; tempindex<total; tempindex++){
			...
			for(var i=0;i<2;i++){
				if(2*tempindex+i>=blogs.length)break;	//此时说明所有图片都添加完毕
				html += '<div id="blog'+blogs[i].id+'" class="blog-item"><div class="blog-header">'+blogs[i].title
				+'<span class="date">'+blogs[i].date+'</span></div><div class="blog-content">'+blogs[i].content+'</div></div>';
			}
			html+='</div>';
			bloglist.innerHTML += html;
		}
		if(html==""){
			html = "您还没有写过日志哦！点击写日志，轻松上传！";
		}
		//get("blog").innerHTML = html;
		//if(photos.length>6)
		//	get("listMore").style.display = 'block';
		
		//在id为RJ-pagebar的div中创建页码条，总页数不大于7的时候显示页数和总页数相等，大于7时显示页数为7
		if((blogs.length/2)<=7){
			creatPageBar("RJ-pagebar", Math.ceil(blogs.length/2),lastindex,Math.ceil(blogs.length/2),show_page);
		}else{
			creatPageBar("RJ-pagebar", blogs.length/2,lastindex,7,show_page);
		}
	}
}
