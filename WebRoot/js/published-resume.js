...
function list_enterprise_resume(){
	createXmlHttp();
	xmlHttp.open("GET", "published_resume.action",true);			
	xmlHttp.onreadystatechange = responseTo;
	xmlHttp.send(null);
}
function list_published_resume(id){
	createXmlHttp();
	xmlHttp.open("GET", "findEmploymentById_.action?id="+id,true);			
	...
	if(xmlHttp.readyState==4){
		...
		if(records.length==0){
			...
			for(var tempindex=0; tempindex<total; tempindex++){
				...
				html += "<table id=\"published_resume\">";//--------------
				html += "<caption>当前共发布"+records.length+"份招聘信息</caption>";
				html += "<th>招聘信息编号</th><th>发布时间</th><th>截止日期</th><th>标题</th>";
				for(var i=6;i>=0;i--){
					if(calc == -1)
						break;
					
				html += "<tr><td>"+(calc+1)+"</td>" +
				"<td>"+records[calc].date+"</td>" +
				"<td>"+records[calc].deadline+"</td>" +
				"<td><a href='javascript:list_published_resume("+records[calc].id+")' " +
						"class='download'>"+records[calc].title+"</a></td><tr>";
...
function responseTo1(){
	if(xmlHttp.readyState==4){
		...
		if(records.length==0){
			...
			displayCover();
			html += "<th>标题</th><th>"+records.title+"</th>";
			html += "<tr><td>内容</td><td>"+records.content+"</td><tr>";
			html += "<tr><td>发布时间</td><td>"+records.date+"</td><tr>";
			html += "<tr><td>截止日期</td><td>"+records.deadline+"</td><tr>";
			get("pub_resume").innerHTML = html;
		}
	}
}
